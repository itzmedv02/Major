...
import { CloudRain, Droplets, Calendar } from "lucide-react";

const PredictionResult = ({ state, subdivision, year }) => {
  // Simulated prediction values (in a real app, this would come from your ML model)
  const predictedAnnual = Math.floor(Math.random() * 1000) + 2500;
  const predictedMonsoon = Math.floor(predictedAnnual * 0.6);
  const confidence = (Math.random() * 5 + 90).toFixed(1);

  const getSeasonalForecast = () => {
    const monthlyData = [
      { season: "Jan-Feb", rainfall: Math.floor(Math.random() * 200) + 100 },
      { season: "Mar-May", rainfall: Math.floor(Math.random() * 600) + 300 },
      { season: "Jun-Sep", rainfall: predictedMonsoon },
      { season: "Oct-Dec", rainfall: Math.floor(Math.random() * 800) + 400 },
    ];
    return monthlyData;
  };

  const seasonalData = getSeasonalForecast();
